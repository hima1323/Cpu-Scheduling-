  ...
  return (
    <div className="glass rounded-2xl p-6 animate-slide-up">
      <div className="flex items-center gap-3 mb-5">
        <TrendingDown size={20} className="text-accent" style={{ color: '#6366f1' }} />
        <h2 className="text-lg font-bold text-white">Quantum Analyzer</h2>
        <span className="text-slate-400 text-sm">Round Robin q = 1 → 50</span>
      </div>

      {/* Controls */}
      <div className="flex flex-wrap gap-3 mb-6 p-4 rounded-xl bg-[#0d0d1a] border border-[#2a2a3d]">
      ...
      {optimal && (
        <div className="flex gap-3 mb-5 flex-wrap">
          <div className="stat-card flex-1 min-w-[130px]">
            <div className="text-xs text-slate-500 mb-1">Optimal Quantum</div>
            <div className="text-2xl font-bold" style={{ color: '#22d3ee' }}>q = {optimal.quantum}</div>
          </div>
          <div className="stat-card flex-1 min-w-[130px]">
            <div className="text-xs text-slate-500 mb-1">Min Avg Wait</div>
            <div className="text-2xl font-bold" style={{ color: '#6366f1' }}>{optimal.avg_waiting_time.toFixed(2)} ms</div>
          </div>
          <div className="stat-card flex-1 min-w-[130px]">
            <div className="text-xs text-slate-500 mb-1">Min Ctx Switches</div>
        ...
        <div style={{ height: 320 }}>
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={sweepData} margin={{ top: 8, right: 16, left: 0, bottom: 8 }}>
              ...
              <XAxis dataKey="quantum" stroke="#4b5563" tick={{ fill: '#94a3b8', fontSize: 11 }}
                label={{ value: 'Time Quantum (q)', position: 'insideBottom', offset: -4, fill: '#64748b', fontSize: 11 }} />
              <YAxis stroke="#4b5563" tick={{ fill: '#94a3b8', fontSize: 11 }} />
              <Tooltip content={<CustomTooltip />} />
              <Legend wrapperStyle={{ fontSize: 12, paddingTop: 8 }} />
              {optimal && (
                <ReferenceLine x={optimal.quantum} stroke="#22d3ee" strokeDasharray="4 3"
                  label={{ value: `Optimal q=${optimal.quantum}`, fill: '#22d3ee', fontSize: 10 }} />
              )}
