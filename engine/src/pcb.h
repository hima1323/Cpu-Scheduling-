...
struct PCB {
    ...
    PCB(int pid, std::string name, double arrival, double burst, int prio = 0, double deadline = 100, int tickets = 10)
        : pid(pid), name(std::move(name)),
          arrival_time(arrival), burst_time(burst),
          remaining_time(burst), priority(prio),
          effective_priority(prio), deadline(deadline), tickets(tickets) {}
};
